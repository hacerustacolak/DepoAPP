 ...
 */
@Component({
	selector: 'interview-type-autocomplete',
	templateUrl: './interview-type-autocomplete.component.html'
})
export class InterviewTypeAutocompleteComponent implements OnInit {
	isLoading = false;
	isErasable = true;
	...
	constructor(private translateService: TranslateService,
		...
		this.placeholder = this.translateService.instant('CRM.MERCHANT.INTERVIEW.TYPE');
	}
	...
	loadOptions(): any {
		this.merchantService.getInterviewTypesAll().subscribe((res: ApiResponseModel<any>) => {
	...
	filterOptions(value: string): any[] {
		...
		return this.options.filter(option => option.interviewType.toLowerCase().includes(value));
	}
}
