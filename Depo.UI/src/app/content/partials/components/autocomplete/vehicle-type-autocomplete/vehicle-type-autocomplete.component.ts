 ...
 */
@Component({
	selector: 'vehicle-type-autocomplete',
	templateUrl: './vehicle-type-autocomplete.component.html',
	providers: [VehicleCapacityService]
})
export class VehicleTypeAutocompleteComponent implements OnInit {
	isLoading = false;
	isErasable = true;
	...
	loadOptions(): any {
		this.vehicleCapatiyService.getTypeAll().subscribe((res: ApiResponseModel<any[]>) => {
	...
	filterOptions(value: string): any[] {
		...
		return this.options.filter(option => option.typeName.toLowerCase().includes(value));
	}
}
