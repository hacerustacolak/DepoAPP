 ...
 */
@Component({
	selector: 'sc-vehicle-type-autocomplete',
	templateUrl: './sc-vehicle-type-autocomplete.component.html',
	providers: [VehicleService]
})
export class SCVehicleTypeAutocompleteComponent implements OnInit {
	isLoading = false;
	isErasable = true;
	...
	constructor(private vehicleService: VehicleService,
		...
		this.placeholder = this.translateService.instant('DEFINITION.VEHICLE.VEHICLE.TYPE');
	}
	...
	loadOptions(): any {
		this.vehicleService.getSCVehicleTypeAll().subscribe((res: ApiResponseModel<any[]>) => {
	...
	filterOptions(value: string): any[] {
		...
		return this.options.filter(option => option.scVehicleName.toLowerCase().includes(value));
	}
}
