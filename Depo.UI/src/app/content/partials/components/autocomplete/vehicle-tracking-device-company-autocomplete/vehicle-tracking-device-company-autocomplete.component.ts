...
import { VehicleService } from '../../../../pages/components/definitions/vehicle/vehicles/vehicle.service';
 ...
 */
@Component({
	selector: 'vehicle-tracking-device-company-autocomplete',
	templateUrl: './vehicle-tracking-device-company-autocomplete.component.html',
	providers: [VehicleService]
})
export class VehicleTrackingDeviceCompanyAutocompleteComponent implements OnInit {
	isLoading = false;
	isErasable = true;
	...
	constructor(private vehicleService: VehicleService,
		...
		this.placeholder = this.translateService.instant('DEFINITION.VEHICLE.VEHICLE.COMPANY');
	}
	...
	loadOptions(): any {
		this.vehicleService.getVehicleTrackingDeviceCompanyAll().subscribe((res: ApiResponseModel<any[]>) => {
	...
	filterOptions(value: string): any[] {
		...
		return this.options.filter(option => option.vehicleTrackingDeviceCompanyName.toLowerCase().includes(value));
	}
}
