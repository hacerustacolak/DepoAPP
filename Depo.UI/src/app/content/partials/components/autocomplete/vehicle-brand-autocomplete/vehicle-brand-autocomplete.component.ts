...
import { VehicleBrandService } from '../../../../pages/components/definitions/vehicle/vehicleBrand/vehicleBrand.service';
import { VehicleBrandModel } from '../../../../pages/components/definitions/vehicle/vehicleBrand/vehicleBrand.model';
 ...
 */
@Component({
	selector: 'vehicle-brand-autocomplete',
	templateUrl: './vehicle-brand-autocomplete.component.html',
	providers: [VehicleBrandService]
})
export class VehicleBrandAutocompleteComponent implements OnInit {
	isLoading = false;
	isErasable = true;
	...
	constructor(private vehicleBrandService: VehicleBrandService,
		...
		this.placeholder = this.translateService.instant('DEFINITION.VEHICLE.BRAND.NAME');
	}
	...
	loadOptions(): any {
		this.vehicleBrandService.getAll().subscribe((res: ApiResponseModel<VehicleBrandModel[]>) => {
	...
	filterOptions(value: string): any[] {
		...
		return this.options.filter(option => option.brandName.toLowerCase().includes(value));
	}
}
