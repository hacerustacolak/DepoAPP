 ...
 */
@Component({
	selector: 'warehouse-autocomplete',
	templateUrl: './warehouse-autocomplete.component.html',
	providers: [RegionService]
})
export class WarehouseAutocompleteComponent implements OnInit {
	isLoading = false;
	isErasable = true;
	...
	loadOptions(): any {
		this.regionService.getAllWarehouse().subscribe((res: ApiResponseModel<any[]>) => {
	...
	filterOptions(value: string): any[] {
		...
		return this.options.filter(option => option.WarehouseName.toLowerCase().includes(value));
	}
}
